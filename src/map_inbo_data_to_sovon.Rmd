...
MRI is 0 for observations linked to action `meweg` and no `vang` (a new metal ring is applied when a bird without metal ring is caught):

```{r}
obs_ref_meweg_no_vang <- 
  obs_and_acts %>%
  filter(raw_AktieCode == "meweg") %>%
  distinct(observation_reference) %>%
  anti_join(obs_and_acts %>%
              filter(raw_AktieCode == "vang") %>%
              distinct(observation_reference),
...
obs_and_acts <- 
  obs_and_acts %>%
  mutate(MRI = ifelse(observation_reference %in% obs_ref_meweg_no_vang, 0, MRI))
```
