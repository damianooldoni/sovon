...
We change `PSA-T1` to `Kleine Vlakte`:

```{r}
obs_and_acts <- obs_and_acts %>%
  mutate(raw_PlaatsToponymDetail = if_else((raw_KleurringNummer == "AVOH" & 
                                raw_Nummer == "114756"),
                                "Kleine Vlakte",
                                raw_PlaatsToponymDetail))
```

Now we can remove duplicates in applying color and metal rings:
