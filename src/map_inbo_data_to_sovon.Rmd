...
conn <- DBI::dbConnect(odbc::odbc(),
                       ...
                       server = Sys.getenv("driver"),
                       database = Sys.getenv("D0016_00_Meeuwen"),
                       port = Sys.getenv(1433),
...
We filter the ring actions, `rngkl` and `rngme`, and we retrieve the ringers in `obs` by `raw_WaarnemerNummer`:
...
ringers_number <- obs %>% 
  filter(raw_Nummer %in% obs_numbers) %>%
...
Aggregate information about observation location:

```{r}
obs <- obs %>% mutate(
  observation_location = str_c(raw_PlaatsGemeente,
                               raw_PlaatsToponym, 
                               sep = ", in de omgeving van "
)) %>%
  mutate(observation_location = str_c(observation_location, 
