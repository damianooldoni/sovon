...
We can now close the connection to server:
...
The INBO seabird team provided a spreadsheet to map color rings (see section [bird shorthand](#bird-shorthand)). From this spreadhseet we distilled some text files with specific mapping information:
...
obs_and_acts <-
  obs_and_acts %>%
  rename_at(vars(starts_with("raw_Metaalring")),
            ~ paste0(., "_obs"))
```
...
crbirding_birds <- 
  ...
  unnest() %>%
  mutate(bird_date_end = as.POSIXct(bird_date_end)) %>%
  select(bird_reference, bird_shorthand, bird_shorthand_pt, 
...
rngme_before_rngkl <- 
  ...
  select(bird_reference, raw_Datum) %>%
  rename(raw_Datum_rngme = raw_Datum) %>%
  left_join(obs_and_acts %>%
              filter(raw_AktieCode == "rngkl") %>%
              select(bird_reference, raw_Datum) %>%
              rename(raw_Datum_rngkl = raw_Datum),
            by = "bird_reference") %>%
  filter(raw_Datum_rngme < raw_Datum_rngkl) %>%
  rename(bird_date_begin = raw_Datum_rngme,
         bird_date_end = raw_Datum_rngkl) %>%
  mutate(bird_shorthand = NA_character_,
         bird_shorthand_pt = NA_character_,
         bird_notes = "bird_shorthand not available.") %>%
  select(bird_reference, 
         bird_shorthand, 
         bird_shorthand_pt,
         everything())
...
crbirding_birds %>%
  filter(bird_reference %in% 
           (test %>% filter(is.na(bird_shorthand)) %>% pull(bird_reference))) %>%
  head(n = 50)
```

### Ring number

The _ring number_, or _metal ring number_, can be found in field `raw_MetaalringNummer`, which contains the **most recent** metal ring number. Recoverying the complete chronology of metal rings for each bird is almost impossible as it is:

1. not complete
2. included in texutal description (field `raw_Opmerking` of `tblWaarneming`)

We can have an idea by selecting observations linked to action `vang` and containing 6 or more consecutive numbers in the notes:

```{r example_notes_changes_metalring}
obs_and_acts %>%
  filter(str_detect(raw_Opmerking, pattern = "[0-9]{6,}")) %>%
  filter(raw_AktieCode == "vang") %>%
  select(raw_Opmerking)
```
...
no_rngme <- 
  ...
  rename(raw_KleurringNummer = raw_Nummer) %>%
...
no_rngme_info <- 
  ...
  left_join(obs_and_acts %>%
            ...
            by = c("bird_reference", "raw_KleurringNummer")) %>%
  mutate( raw_AktieCode = "rngme")
no_rngme_info
```
...
and check that all birds (`bird_reference`) with a valid metal ring are linked to one and only one `rngme` action. Exceptions:
...
Few examples (`bird_reference` 74 and 75 refer to birds which got metal ring first, see previous section):

```{r}
crbirding_birds %>%
  filter(bird_reference %in% c(8999, 9028, 74, 75))
```
...
This is how `bird_notes` has been updated, limited to birds where changes could happen:
...
Retrieving the end date of metal rings is challenging: INBO experts informed us that the information in `raw_MetaalringNummer` refers **always** to the **most recent** ring. Some rings have been changed during observations related to action `vang` with the old ring number in field `raw_Opmerking` as textual annotation. However, the databases of INBO and SOVON are conceived around colour rings and information about the metalring is less relevant. The INBO experts suggested to not bother about the various metal ring codes that a bird had during its life. See [issue 34](https://github.com/inbo/sovon/issues/34).
...
Bird age at the moment of any observation can be found in column `raw_LeeftijdCode` of `obs_and_acts`. Values present:

```{r show_present_LeeftijdCode}
obs_and_acts %>%
  distinct(raw_LeeftijdCode)
```
...
obs_and_acts <- 
  obs_and_acts %>%
  mutate(bird_age_obs = recode(raw_LeeftijdCode,
    "PU" = "1",
    "AD" = "A",
    "J1" = "3",
    "I4" = "5",
...
We remove the help column `raw_AktieCode` from `crbirding_birds` and rename  `bird_age_obs` to `bird_age_ringing`:

```{r remove_raw_AktieCode}
crbirding_birds <- 
  crbirding_birds %>%
  select(-raw_AktieCode) %>%
  rename(bird_age_ringing = bird_age_obs)
```
...
obs_and_acts %>%
  ...
  select(bird_reference, raw_Nummer, raw_Datum, raw_AktieCode) %>%
  arrange(raw_Datum)
```

### Observation date

The date is saved in column `raw_Datum`. We copy it in required column `observation_date`:

```{r obs_date}
obs_and_acts <- 
  obs_and_acts %>% 
  mutate(observation_date = raw_Datum)
```
...
obs_and_acts <- 
  ...
  mutate(
    observation_location = ifelse(!is.na(raw_PlaatsToponymDetail),
...
obs_and_acts <- 
  obs_and_acts %>%
  mutate(melder_ringersnummer = raw_WaarnemerNummer)
```
...
obs_and_acts <- obs_and_acts %>%
  left_join(
    users %>% 
      select(raw_Nummer, user_first_name, user_last_name),
...
The observation reference is an unique identifier (number) assigned to each observation, which could entail multiple actions. This field exists already: `raw_Nummer`.

```{r obs_ref}
obs_and_acts <- 
  obs_and_acts %>% 
  mutate(observation_reference = raw_Nummer)
```
...
obs_and_acts %>%
  ...
  select(observation_reference, bird_reference, raw_AktieCode, MRI) %>%
  head(n = 50)
```
...
obs_ref_me_klweg <- 
  obs_and_acts %>%
  filter(raw_AktieCode %in% c("me", "klweg")) %>%
  pull(observation_reference)
...
vang_notes <- 
  ...
  filter(str_detect(raw_Opmerking, pattern = "[0-9]{6,}")) %>%
  filter(raw_AktieCode == "vang") %>%
  select(observation_reference, raw_Opmerking, bird_reference, raw_Datum) %>%
  arrange(bird_reference)
vang_notes
```
...
obs_ref_assign_last_MRI <- obs_and_acts_assign_last_MRI %>%
  distinct(observation_reference) %>%
  pull(observation_reference)
obs_and_acts <-
  obs_and_acts %>%
  filter(! observation_reference %in% obs_ref_assign_last_MRI) %>%
  bind_rows(obs_and_acts_assign_last_MRI) %>%
  arrange(bird_reference, observation_date)
obs_and_acts
```
...
obs_and_acts %>%
  mutate(condition = case_when(
    raw_AktieCode == "dood" ~ 1,
    raw_AktieCode %in% c("vang", "vangl") ~ 8,
    raw_AktieCode == "veld" ~ 7,
    raw_AktieCode == "ziek" ~ 4
))
```
...
We copy the notes in `raw_Opmerking` to SOVON field  `observation_notes`:

```{r obs_notes}
obs_and_acts <- 
  obs_and_acts %>% 
  mutate(observation_notes = raw_Opmerking)
```
