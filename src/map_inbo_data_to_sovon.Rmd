...
Whie waiting for an official answer from INBO experts (see [issue 36](https://github.com/inbo/sovon/issues/36), we solve this lack of uniqueness by discarding the first row:

```{r remove_wrong_one}
birds <-
  birds %>%
  filter(raw_NummerNieuw != "MKAU")
```
...
We solve the uniqueness issue:

```{r remove_last_one}
obs_and_acts <- 
  obs_and_acts %>%
  filter(raw_KleurringNummer != "GPS28" | raw_AktieCode != "rngme")
```
