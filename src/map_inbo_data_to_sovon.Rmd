...
We import the color rings in new column `first_Nummer`. For all other rings will be `first_nummer = raw_Nummer` as they are the very first rings. We will also add note `'uvabits_gps_tagged.'`

```{r map_to_first_Nummer}
map_gps_add_notes <- map_gps %>%
  mutate(bird_notes = "uvabits_gps_tagged.")
birds <- birds %>%
...
As suggested by experts, it woudl be handy to add bird name in `bird_notes` as well. The bird names can be got by reading this [file](https://raw.githubusercontent.com/inbo/bird-tracking/master/data/interim/individuals.csv); The names are saved in column `individual_remarks`:
...
Add names to `bird_notes`:

```{r add_names_to_notes}
birds <- 
  birds %>%
  left_join(uvabits_names %>%
              select(ring_number, individual_remarks) %>%
              # add point at the end of name for possible extra notes in future
              mutate(individual_remarks = paste0(individual_remarks, ".")), 
            by = c("raw_MetaalringNummer" = "ring_number")) %>% 
  replace_na(list(individual_remarks = "", bird_notes = "")) %>% 
 unite(col = "bird_notes", 
       bird_notes, individual_remarks,
       sep = "", remove = TRUE)
```

So, for example the bird with `raw_Nummer` equal to GP130 is mapped as follows:
