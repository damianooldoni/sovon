...
The _ring number_, or _metal ring number_, can be found in field `raw_MetaalringNummer`, which contains the **most recent** metal ring number. A ring number should consist of up to ten **alphanumeric** characters. At page 7 of the [EURING Exchange Code 2000+ document](https://euring.org/files/documents/E2000PLUSExchangeCodev1161.pdf) we read the following instructions:
...
We add an id number to each observation/action:
...
The ring number is equal to `bird_shorthand_pt` in `crbirding_birds`. We do it via joining on `bird_reference`. In case of multiple rings, the choice of `bird_shorthand` is based on a comparison between the date of the observation and the dates `bird_date_begin` (date of applying color ring) and `bird_date_end` (date of removing color ring to apply a new one):

```{r obs_color_rings}
obs_color_rings <-
  obs_and_acts %>%
  left_join(crbirding_birds %>%
              select(bird_reference, bird_shorthand_pt, 
                     bird_date_begin, bird_date_end),
            by = "bird_reference") %>%
  filter(raw_Datum >= bird_date_begin & 
            (is.na(bird_date_end) | raw_Datum < bird_date_end)) %>%
  mutate(ring_number = bird_shorthand_pt)
```

Examples:

```{r examples_ring_number_not_NA}
obs_color_rings %>% 
  select(observation_reference, bird_reference,
         raw_Datum, bird_date_begin, bird_date_end,
         ring_number) %>%
  head(n = 10)
```

These observations are still a subset of all observations:

```{r show_obs_color_rings_subset_obs_and_acts}
nrow(obs_color_rings) < nrow(obs_and_acts)
```

The reason is that some birds got a color ring **after** the metal ring was applied. Some examples:

```{r obs_without_color_ring}
# retrieve date of applying first color ring
obs_and_acts_rngkl <- obs_and_acts %>%
  filter(raw_AktieCode == "rngkl") %>%
  select(bird_reference, raw_Datum)

# observations with no color ring
obs_only_metal_rings <-
  obs_and_acts %>%
  anti_join(obs_color_rings) %>%
  left_join(obs_and_acts_rngkl %>%
              rename(datum_rngkl = raw_Datum),
            by = c("bird_reference")) %>%
  filter(raw_Datum < datum_rngkl)

obs_only_metal_rings %>% head(n = 10)
```

In all these cases the field `ring_number` should be left empty:

```{r assign_ring_number_as_NA}
obs_only_metal_rings <-
  obs_only_metal_rings %>%
  mutate(ring_number = NA_character_)
```

Check whether we have a `ring_number` for all observations:

```{r same_n_rows}
nrow(obs_and_acts) == nrow(obs_color_rings) + nrow(obs_only_metal_rings)
```

and that no duplicates have been created:

```{r check_no_duplicates_while_making_ring_number}
bind_rows(obs_color_rings, obs_only_metal_rings) %>% 
  distinct(observation_reference) %>%
  nrow() == nrow(obs_and_acts)
```

Then we can add field `ring_number` to  `obs_and_acts`:

```{r assign_ring_number}
obs_and_acts <- 
  obs_and_acts %>%
  left_join(
    bind_rows(obs_color_rings %>% select(-starts_with("bird_date_")), 
              obs_only_metal_rings) %>%
      select(observation_reference, ring_number),
