...
MRI is 4 for observations linked to action `me`:

```{r MRI_4}
obs_ref_me <- 
  obs_and_acts %>%
  filter(raw_AktieCode == "me") %>%
  pull(observation_reference)

obs_and_acts <- 
  obs_and_acts %>%
  mutate(MRI = ifelse(observation_reference %in% obs_ref_me, 4, MRI))
```
...
obs_and_acts %>%
  filter(observation_reference %in% obs_ref_me) %>%
  select(observation_reference, bird_reference, raw_AktieCode, MRI) %>%
  head(n = 50)
```
