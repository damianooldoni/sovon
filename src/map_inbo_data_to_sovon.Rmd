...
We check there are no duplicates in `obs_actions`, that means no multiple lines for each distinct couple `raw_WaarnemingNummer` - `raw_AktieCode`:
...
Observations without action code:
...
In order to ease the mapping of birds and observations, we add action to observations in order to have a unique **tidy** data frame:
...
Filtering metal rings without any number results in detecting anomalies:
...
Value `none` means _not metal-ringed bird_. From the EUring documentation:

> If a bird is not metal-ringed on first capture but is identified in another way (primary identification method not A0), a unique number relating to that bird must be entered in this field. That unique number will represent a not-present-metal-ring-number. It must be a unique number because this identification number is unique for database reasons. That unique number must be allocated by the national scheme. It is recommended to start that unique number with the characters “NMR” (meaning “not metal ring”) and then followed by seven numbers. For example: NMR0000001. It is recommended that the Scheme and ringer agree this unique number that will be used to identify the bird throughout its life. 

Ring number should also not contain any `*`. We remove all asteriks:

```{r remove_*_in_ring_number}
birds <- birds %>%
  mutate(bird_ring_number = gsub("\\*", "", raw_MetaalringNummer))
```
