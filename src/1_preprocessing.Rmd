...
Same holds true for observation after `klgev`:
...
obs_after_klgev <-
  ...
  filter(klgev == "klgev") %>%
  select(KleurringNummer, Datum) %>%
  rename(klgev_datum = Datum) %>%
  left_join(obs_and_acts, by = "KleurringNummer") %>%
  filter(klgev_datum < Datum) %>%
  select(Nummer, KleurringNummer, Datum, klgev_datum, acts, everything()) %>%
  arrange(KleurringNummer, Datum)
```

```{r remove_obs_after_klgev}
obs_and_acts<- 
  obs_and_acts %>%
  filter(!Nummer %in% obs_after_klgev$Nummer)
```
