...
exceptions_age <- 
  ...
  left_join(obs_and_acts %>%
              filter(!is.na(LeeftijdCode)) %>%
              select(Nummer, KleurringNummer, Datum, WaarnemerNummer,
...
Are these exceptions field observations (`veld` action)? 

```{r exceptions_all_field_obs}
exceptions_age %>%
  filter(veld == "veld") %>%
  nrow == nrow(exceptions_age)
```

If yes, then we leave them as they are related to age estimations as given by observers.
