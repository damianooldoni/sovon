...
We also extract the ring places:

```{r  get_ring_place_table}
ring_place_table <- dbGetQuery(conn, "SELECT * FROM dbo.cdeRingPlaats") %>%
  as_tibble()
ring_place_table
```
...
write_tsv(
  ring_place_table, 
  path = here::here("data", "input", "ring_place_table.tsv"), 
  na = ""
)
```
