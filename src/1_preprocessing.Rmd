...
conn <- dbConnect(odbc::odbc(), driver = Sys.getenv("driver"), 
    server = Sys.getenv("server"), database = Sys.getenv("database"), 
    port = Sys.getenv("port"), trusted_connection = Sys.getenv("tc"))
```
...
```{r get_color_table}
colors_table <- dbGetQuery(conn, "SELECT * FROM dbo.cdeKleur") %>%
  as_tibble()
colors_table
```
...
We save the tables containing action codes and colors in `./data/input` as it could be useful in following mapping steps:
