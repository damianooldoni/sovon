...
Birds getting color and metal ring at the same time and never ringed twice:
...
birds %>%
  distinct(ring_color, RingKleurCode)
```
...
We assign them color blue as ring color and add note `virtual_ring.` to `sovon_bird_notes` as decided in [#81](https://github.com/inbo/cr-birding/issues/81#issuecomment-479365880):
...
crbirding_birds <-
  ...
  mutate(
    ...
    sovon_bird_shorthand_color = if_else(
      sovon_bird_shorthand %in% rings_color_still_unknown_with_B_third_pos$sovon_bird_shorthand,
      paste0("B-", sovon_bird_shorthand_color),
      sovon_bird_shorthand_color
    ),
    sovon_bird_shorthand_pt_color = if_else(
      sovon_bird_shorthand %in% rings_color_still_unknown_with_B_third_pos$sovon_bird_shorthand,
      paste0("B-", sovon_bird_shorthand_pt_color),
      sovon_bird_shorthand_pt_color
    )
  )
```
...
```{r read_names}
uvabits_file <- "https://github.com/inbo/bird-tracking/blob/8a7a2c91d54b486d90a97f85cfaa30cede67315d/data/interim/individuals.csv"
uvabits_names <-
