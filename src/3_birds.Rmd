...
We have to solve the lack of unicity discussed in preprocessing, which ends up in multiple `sovon_bird_reference` for the same bird. See here the few issues encountered:
...
We assign the lower `sovon_bird_reference`:
...
The reconstruction of the series of color rings for each bird is made by using columns `first_Nummer`, `NummerNieuw` and the information from spreadsheets of experts. We will collect the needed information in new dataframe, called `crbirding_birds` which will grow up to become the end product of color ring table.

First step is to *gather* `first_Nummer`  and `NummerNieuw` in new column `bird_shorthand`. We do it and we create a new data.frame called `crbirding_birds` which will be the end product related to rings:

```{r backbone_mapping_shorthand}
crbirding_birds <- 
  birds %>%
  select(sovon_bird_reference, first_Nummer, 
         NummerNieuw, NummerDesc, bird_notes) %>%
  gather(key = col_nummer,
         value = bird_shorthand,
         first_Nummer, NummerNieuw) %>%
  select(-col_nummer) %>%
  select(sovon_bird_reference, bird_shorthand, NummerDesc, bird_notes)
```

We can also assign the color ring version with point (`bird_shorthand_pt`) by matching columns `bird_shorthand` and `NummerDesc` where possible. Note that `bird_shorthand_pt` may contain points, but for old rings it is equal to `bird_shorthand`:

```{r map_shorthand_with_point}
crbirding_birds <-
  crbirding_birds %>%
  mutate(bird_shorthand_pt = ifelse(
    str_remove_all(string = NummerDesc, pattern = "\\.") == bird_shorthand,
    NummerDesc,
    bird_shorthand)
    ) %>%
  distinct() %>%
  select(sovon_bird_reference, bird_shorthand, bird_shorthand_pt, bird_notes) %>%
  # to map correctly MKAU (sovon_bird_reference: 3716)
  group_by(sovon_bird_reference, bird_shorthand) %>%
...
crbirding_birds %>%
  filter(sovon_bird_reference == 14) %>%
  select(sovon_bird_reference, bird_shorthand, bird_shorthand_pt)
```
...
crbirding_birds %>%
  filter(sovon_bird_reference == 3716) %>%
  select(sovon_bird_reference, bird_shorthand, bird_shorthand_pt)
```

## Bird ringing date

The date of ringing (to be mapped as `bird_date_begin`) can be found in data frame `obs_and_acts` in the column `Datum` relatively to code action `rngkl` (code action of applying colour ring).

```{r example_rngkl_actions}
obs_and_acts %>% 
  filter(AktieCode == "rngkl") %>%
  select(sovon_bird_reference, first_Nummer, 
         Datum, AktieCode) %>%
  head(n = 10)
```
...
exceptions_one_bird_one_rngkl <- obs_and_acts %>%
  ...
  left_join(obs_and_acts %>%
              select(sovon_bird_reference, Datum, AktieCode),
            by = "sovon_bird_reference") %>%
  filter(AktieCode == "rngkl") %>%
  left_join(crbirding_birds %>%
              select(sovon_bird_reference, bird_shorthand),
            by = "sovon_bird_reference") %>%
  distinct() %>%
  select(-n)
...
crbirding_birds <- 
  ...
  summarize(bird_date_begin = min(Datum)) %>%
...
crbirding_birds %>%
  ...
  arrange(sovon_bird_reference, bird_date_begin)
```
...
birds_two_rings <- 
  ...
  count() %>%
  filter(n == 2)
all(birds_two_rings$sovon_bird_reference == birds_multiple_rings$sovon_bird_reference)
```
...
bird_one_vang <- obs_and_acts %>%
  filter(sovon_bird_reference %in% birds_multiple_rings$sovon_bird_reference,
         AktieCode %in% c("vang", "vangl")) %>%
  select(sovon_bird_reference, Datum) %>%
  group_by(sovon_bird_reference) %>%
  count() %>%
  filter(n == 1) %>%
  left_join(obs_and_acts %>%
              select(sovon_bird_reference, Datum, AktieCode),
            by = "sovon_bird_reference") %>%
  filter(AktieCode %in% c("vang", "vangl")) %>%
  rename("bird_date_begin" = "Datum") %>%
...
bird_one_vang_with_date <- 
  ...
  select(-c(n, AktieCode))
crbirding_birds <- 
  crbirding_birds %>%
  anti_join(bird_one_vang_with_date,
            by = c("sovon_bird_reference", "bird_shorthand")) %>%
...
bird_two_vang_get_date <- 
  ...
  left_join(bird_exceptions %>%
              select(-bird_date_begin), by = "sovon_bird_reference") %>%
  select(-c(n, AktieCode)) %>%
...
intermediate_rings <- 
  intermediate_rings %>%
  left_join(crbirding_birds %>%
              select(sovon_bird_reference, bird_shorthand, bird_notes),
...
```{r}
crbirding_birds %>% filter(sovon_bird_reference == 7591)
```
...
rngme_before_rngkl <- 
  ...
  rename(bird_date_begin = Datum_rngme,
         bird_date_end = Datum_rngkl) %>%
...
birds %>% 
  distinct(MetaalringNummer, bird_ring_number) %>%
  head(n = 200)
```
...
birds %>%
  select(sovon_bird_reference, bird_ring_number) %>%
  group_by(sovon_bird_reference) %>%
  count() %>%
  filter(n > 1) %>%
  left_join(birds) %>%
...
obs_and_acts  %>%  
  filter(AktieCode == "rngme") %>%
  select(sovon_bird_reference, KleurringNummer, Datum, AktieCode) %>%
  head(n = 20)
```
...
obs_and_acts %>%
  ...
  left_join(obs_and_acts %>%
              filter(AktieCode == "rngme") %>%
              select(sovon_bird_reference, KleurringNummer, 
                     Datum, AktieCode),
            by = "sovon_bird_reference") %>%
  select(-n)
```
...
Few examples (`sovon_bird_reference` 74 and 75 refer to birds which got metal ring first, see previous section):
...
Add note about absence of metal ring based on original values in column `MetaaalringNummer` and the fields `sovon_bird_reference`, `bird_ring_number`:
...
crbirding_birds <-
  ...
  mutate(bird_notes = case_when(
     ...
     is.na(bird_ring_number) & MetaalringNummer == "none" ~ 
      ifelse(is.na(bird_notes),
...
Retrieving the end date of metal rings is challenging: INBO experts informed us that the information in `MetaalringNummer` refers **always** to the **most recent** ring. Some rings have been changed during observations related to action `vang` with the old ring number in field `Opmerking` as textual annotation. However, the databases of INBO and SOVON are conceived around colour rings and information about the metalring is less relevant. The INBO experts suggested to not bother about the various metal ring codes that a bird had during its life. See [issue 34](https://github.com/inbo/sovon/issues/34).
...
crbirding_birds <-
  ...
  left_join(birds %>% 
            ...
            by = "sovon_bird_reference") %>%
  mutate(bird_euring = recode(EuringCode,
                              "5920" = "05920",
                              "5910" = "05910",
                              "5926" = "05926",
                              "5922" = "05922",
...
```{r comparison_euring}
crbirding_birds %>% 
  distinct(EuringCode, bird_euring)
```
...
crbirding_birds <- 
  ...
  left_join(birds %>% 
            ...
            by = "sovon_bird_reference") %>%
  mutate(bird_scheme = recode(MetaalringLandCode,
                              "BE " = "BLB",
                              "FR" = "FRP",
                              "NL" = "NLA",
                              "PT" = "POL",
...
```{r comparison_scheme}
crbirding_birds %>% distinct(MetaalringLandCode, bird_scheme)
```
...
crbirding_birds <- 
  ...
  left_join(birds %>% 
            ...
            by = "sovon_bird_reference") %>%
  mutate(bird_sex = recode(GeslachtCode,
...
```{r comparison_sex}
crbirding_birds %>% distinct(GeslachtCode, bird_sex)
```
...
We remove the help column `AktieCode` from `crbirding_birds` and rename  `bird_age_obs` to `bird_age_ringing`:
...
bird_ref_uvabits <- 
  ...
  select(sovon_bird_reference, bird_name)
```
...
# Create `crbirding_birds`

SOVON is interested in color rings versions with a dot if dots are present, even if the dots are for readibility reasons. At the end of the mapping we can overwrite `bird_shorthand` with control column `bird_shorthand_pt`:
...
write_tsv(crbirding_birds, path = here("data", "processed", "crbirding_birds.tsv"), na = "")
```
