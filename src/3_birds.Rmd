...
```{r add_date_birds_with_two_vang}
crbirding_birds <-
  crbirding_birds %>%
...
crbirding_birds <-
  bind_rows(
    crbirding_birds %>%
      filter(!sovon_bird_shorthand %in% c("FAAG", "YCAF", "FHOV", "BUAH")),
    info_faag,
    info_ycaf,
    info_fhov,
    info_buah
...
We add a temporary patch in order to solve [issue #15-issuecomment-700596629](https://github.com/inbo/cr-birding/issues/15#issuecomment-700596629). We remove ring history and observations of birds where no date of ringing is present:

```{r add_patch_norngkl}
no_date_bird_reference <- 
  crbirding_birds %>%
  filter(is.na(sovon_bird_date_begin)) %>%
  distinct(sovon_bird_reference)
crbirding_birds <- 
  crbirding_birds %>%
  filter(!sovon_bird_reference %in% no_date_bird_reference)
obs_and_acts <-
  obs_and_acts %>%
  filter(! sovon_bird_reference %in% no_date_bird_reference)
```
...
In order to better follow the mapping of metal AND color rings, SOVON suggested to add a column called `sovon_bird_rings_changed`, as shown in this [GitHub comment](https://github.com/inbo/cr-birding/issues/47#issuecomment-469256635).
...
```{r show_changes_rngme_before_rngkl}
crbirding_birds %>%
...
```{r check_n_rings_without_sovon_bird_rings_changed}
crbirding_birds %>%
  filter(is.na(sovon_bird_rings_changed))
```
...
```{r final_overview_crbiridng_birds_n_higher_than_4}
crbirding_birds %>%
  group_by(sovon_bird_reference) %>%
  add_tally() %>%
  filter(n >= 5) %>%
...
For example, B-BBAP is a dark blue ring (see [issue #67](https://github.com/inbo/cr-birding/issues/67#issuecomment-643954772)).
...
```{r add_incsription_color_as_note}
crbirding_birds <- 
  crbirding_birds %>%
...
INBO experts explained us that the rings containing a `B` at third position are *virtual* rings (see [issue #81#issuecomment-478954317](https://github.com/inbo/cr-birding/issues/81#issuecomment-478954317)). It has been suggested to remove them and not exporting them (see [issues #81#issuecomment-698964036](https://github.com/inbo/cr-birding/issues/81#issuecomment-698964036)). We remove them:

```{r remove_virtual_rings}
crbirding_birds <- 
  crbirding_birds %>%
  filter(!
...
We assign fictive color to them to ease the submission to cr-birding database. We assign them blue as ring color (see [issue #84](https://github.com/inbo/cr-birding/issues/84#issuecomment-479443389)):
...
For more details, see [issue #61](https://github.com/inbo/cr-birding/issues/61#issuecomment-472862064).
...
```{r n_rings_of_birds_without_ring}
crbirding_birds %>%
  group_by(sovon_bird_reference) %>%
  add_tally() %>%
  ungroup() %>%
...
This is considered not a problem by SOVON ITer (see [113#issuecomment-605295649](https://github.com/inbo/cr-birding/issues/113#issuecomment-605295649)).

Birds without `sovon_bird_age_obs` while ringing once should be checked:

```{r birds_ringed_once_without_sovon_bird_age_obs}
crbirding_birds %>%
  group_by(sovon_bird_reference) %>%
  add_tally() %>%
  ungroup() %>%
...
Some birds, the ones with a GPS tracker, have a name saved in [this file](https://github.com/inbo/bird-tracking/blob/8a7a2c91d54b486d90a97f85cfaa30cede67315d/data/interim/individuals.csv). The names are saved in column `individual_remarks`:
