...
Some examples from birds ringed thrice:
...
Based on INBO experts knowledge, we can assign color of rings and insriptions for a significant number of old, changed rings as well. All rings with a 4-letter long inscription where the third letter is an A are blue rings with white inscription (see https://github.com/inbo/cr-birding/issues/84#issuecomment-478938321).

Number of rings still not mapped satisfying this condition:
...
crbirding_birds <- 
  ...
  mutate(
    sovon_bird_shorthand_color = if_else(
      !str_detect(sovon_bird_shorthand_color, "\\(") & # ring is not yet mapped
        str_detect(sovon_bird_shorthand_color, "[A-Z]") & # letters only
        str_length(sovon_bird_shorthand_color) == 4 & # 4 characters
        str_sub(sovon_bird_shorthand_color, 3, 3) == "A", # 3rd character is an A
      paste0("BW(", sovon_bird_shorthand_color, ")"),
      sovon_bird_shorthand_color),
    sovon_bird_shorthand_pt_color = if_else(
      !str_detect(sovon_bird_shorthand_pt_color, "\\(") & # ring is not yet mapped
        str_detect(sovon_bird_shorthand_pt_color, "[A-Z]") & # letters only
        str_length(sovon_bird_shorthand_pt_color) == 4 & # 4 characters
        str_sub(sovon_bird_shorthand_pt_color, 3, 3) == "A", # 3rd character is an A
...
The majority of these rings either contain numbers: 

```{r not_coloured_rings_numbers}
rings_color_still_unknown_with_number <- 
  rings_color_still_unknown %>%
  filter(str_detect(sovon_bird_shorthand, "[0-9]"))
rings_color_still_unknown_with_number
```

or have a B at third position:
...
Rings not satsfying these two conditions:

```{r not_coloured_rings_others}
rings_color_still_unknown_left <- 
  rings_color_still_unknown %>%
  filter(!sovon_bird_shorthand %in% 
           rings_color_still_unknown_with_number$sovon_bird_shorthand & 
           !sovon_bird_shorthand %in% 
           rings_color_still_unknown_with_B_third_pos$sovon_bird_shorthand)
rings_color_still_unknown_left
```

INBO experts explained us that the rings containing a `B` at third position are *virtual* rings. We assign them blue as ring color, white as inscription color and add note `virtual_ring.`:
...
crbirding_birds <- 
  ...
  mutate(
    sovon_bird_notes = case_when(
      (!str_detect(sovon_bird_shorthand_pt_color, "\\(") & # ring is not yet mapped
         str_detect(sovon_bird_shorthand, "[A-Z]") & # letters only
         str_length(sovon_bird_shorthand_color) == 4 & # 4 characters
         str_sub(sovon_bird_shorthand_color, 3, 3) == "B") # 3rd char is B or O
    ...
    sovon_bird_shorthand_color = if_else(
      !str_detect(sovon_bird_shorthand_color, "\\(") & # ring is not yet mapped
        str_detect(sovon_bird_shorthand_color, "[A-Z]") & # letters only
        str_length(sovon_bird_shorthand_color) == 4 & # 4 characters
        str_sub(sovon_bird_shorthand_color, 3, 3) == "B", # 3rd char is a B
      paste0("BW(", sovon_bird_shorthand_color, ")"),
      sovon_bird_shorthand_color),
    sovon_bird_shorthand_pt_color = if_else(
      !str_detect(sovon_bird_shorthand_pt_color, "\\(") & # ring is not yet mapped
        str_detect(sovon_bird_shorthand_pt_color, "[A-Z]") & # letters only
        str_length(sovon_bird_shorthand_pt_color) == 4 & # 4 characters
        str_sub(sovon_bird_shorthand_pt_color, 3, 3) == "B", # 3rd char is a B
      paste0("BW(", sovon_bird_shorthand_pt_color, ")"),
      sovon_bird_shorthand_pt_color)
)
```
...
The others rings are foreign rings: no color will be assigned to them.

We can now overwrite the columns `sovon_bird_shorthand` and `sovon_bird_shorthand_pt` and remove the help columns  `sovon_bird_shorthand_color`,  `sovon_bird_shorthand_pt_color` and `color_combination`:
...
ring_position_table <-
  ring_position_table %>%
  mutate(sovon_bird_position = case_when(
    str_sub(Code, start = 1, end = 1) == "L" ~ "LL",
    str_sub(Code, start = 1, end = 1) == "R" ~ "RL",
    TRUE ~ NA_character_),
    sovon_bird_direction = case_when(
    str_sub(Code, start = -1, end = -1) == "D" ~ "D",
    str_sub(Code, start = -1, end = -1) == "U" ~ "U",
    TRUE ~ NA_character_)) %>%
  select(Code, 
         sovon_bird_position, 
         sovon_bird_direction,
         everything())
ring_position_table
```
...
birds %>%
  distinct(Plaats, sovon_bird_position, sovon_bird_direction)
```

Map these values from `birds` to `crbirding_birds`, taking into account that the fields `sovon_bird_position` and `sovon_bird_direction` are available for the very last ring of each bird:
...
crbirding_birds <-
  ...
  group_by(sovon_bird_reference) %>%
  mutate(
    sovon_bird_position = if_else(
      sovon_bird_date_begin == max(sovon_bird_date_begin),
      sovon_bird_position,
      NA_character_),
    sovon_bird_direction = if_else(
      sovon_bird_date_begin == max(sovon_bird_date_begin),
      sovon_bird_direction,
      NA_character_)
)
...
Some examples from birds ringed more than once:
...
cr_birds_cols <- c(
  ...
  "bird_age_ringing", "bird_position", "bird_direction", "bird_notes"
)
```
