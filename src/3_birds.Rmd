...
We start from the tempoary data saved in TSV files in `data\interim`. 
...
```{r import_temp_birds}
birds <- read_tsv(here::here("data", "interim", "birds.tsv"), guess_max = 50000)
```

## Read observation data

Import `obs_and_actions.tsv`:
...
First step is to *gather* `first_Nummer`  and `NummerNieuw` in new column `bird_shorthand`:
...
The date of applying a ring shuld be mapped as `bird_date_begin`. The date of applying the first ring can be found in data frame `obs_and_acts` in the column `Datum` for actions `rngkl` (code action of applying very first color ring):
...
crbirding_birds %>%
  ...
  filter(bird_shorthand %in% exceptions_one_bird_one_rngkl$bird_shorthand)
```
...
As you cannot change a ring to a bird without catching him, we can use these unique dates as date of ringing, `bird_date_begin`:
