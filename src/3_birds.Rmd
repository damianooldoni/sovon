...
birds %>%
  distinct(Plaats, sovon_bird_ring_position, sovon_bird_direction)
```

Map these values from `birds` to `crbirding_birds`, taking into account that the fields `sovon_bird_ring_position` and `sovon_bird_direction` are available for the very last ring of each bird:
