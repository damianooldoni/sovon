...
INBO experts explained us that all color rings are applied on tarsus, MRI: 2, while metal ring position is on tarsus or above or unknown, MRI: 1. However, the value 2 is not allowed by SOVON (see [comment in issue 47](https://github.com/inbo/cr-birding/issues/47#issuecomment-478926426)). Values allowed: 1, 4 and 5, where 1 is used for adding a color and/or metal ring, while 5 for changing
...
```{r is_capture}
# assign default vallue "N"
obs_and_acts <-
  obs_and_acts %>%
  mutate(sovon_observation_is_capture = "N")
...
In case of patch exports of field observations, the field `sovon_observation_is_capture` should always be `"N"`. Distribution of `sovon_observation_is_capture`:
...
Save to text file (comma separated value). We use suffix `patch` followed by date `20210511`:
...
As we agreed to not add any additional ring in INBO database we limit the patch export to observations only. This means that the only possible changes in bird data are related to the death of a bird or the finding of a (color or metal) ring. In case of death, we add a value to  `bird_date_end` related to the very last ring. In case of finding a ring, a new line should be added.
...
crbirding_birds_dead <-
  ...
  mutate(bird_date_end = if_else(
    ...
    bird_date_end))
crbirding_birds_dead %>%
  relocate(bird_reference, bird_date_begin, bird_date_end)
```

## Add rows for found color rings

We have observations linked to actions `klgev`. This action means that a color ring has been found. This means the bird is not wearing it anymore: we proceed by adding a new row in `crbirding_birds`.
...
We do not go further with  `megev` as not present in patch. See `3_birds.Rmd` for full mapping.
...
Retrieve the bird_reference of the birds whose color  ring has been found:
...
Create `rings_to_add_klgev` by retrieving birds information from `crbirding_birds_2020` and creating new rows as the ring situation changed at each of these observations/actions. We set `bird_rings_changed` = 1, `metal_ring_missing` = FALSE and we leave `bird_shorthand` empty:

```{r show_rings_to_add_klgev}
crbirding_birds_klgev <-
  crbirding_birds_2020 %>%
  filter(bird_reference %in% obs_birds_klgev$sovon_bird_reference)
rings_to_add_klgev <-
  obs_and_acts %>%
  filter(klgev == "klgev") %>%
  select(sovon_bird_reference, Datum) %>%
  rename(bird_reference = sovon_bird_reference) %>%
  left_join(crbirding_birds_klgev, 
            by = "bird_reference") %>%
  group_by(bird_reference) %>%
  filter(bird_date_begin < Datum) %>%
  filter(bird_date_begin == max(bird_date_begin)) %>%
  mutate(
    bird_date_begin = Datum,
    bird_date_end = case_when(
      bird_date_end == bird_date_begin ~ as.POSIXct.Date(NA),
      TRUE ~ bird_date_end
    )
  ) %>%
  select(-Datum) %>%
  mutate(
    bird_rings_changed = 1,
    bird_shorthand = NA_character_,
    bird_notes = NA_character_
  ) %>%
  ungroup() %>%
...
crbirding_birds_klgev <-
  ...
  group_by(bird_reference, bird_date_end) %>%
  mutate(bird_date_end_updated = case_when(
    bird_date_begin == min(bird_date_begin) &
      bird_date_begin != max(bird_date_begin)
    ~ max(bird_date_begin),
    TRUE ~ bird_date_end
  )) %>%
  ungroup() %>%
  mutate(bird_date_end = bird_date_end_updated) %>%
  select(-bird_date_end_updated)
crbirding_birds_klgev
```

## Merge bird data

We proceed now to merge `crbirding_birds_dead` and `crbirding_birds_klgev`.

First, remove the help columns `Datum` and `inscription` from as it is not needed anymore:
...
Check that both `crbirding_birds_dead` and `crbirding_birds_klgev` have some columns:

```{r merge_two_crbirding_birds_dfs}
ncol(crbirding_birds_dead) == ncol(crbirding_birds_klgev)
```

```{r merge_two_crbirding_birds_dfs}
all(names(crbirding_birds_dead) == names(crbirding_birds_klgev))
```

And that these columns are the same as in the  final export:

```{r check_cols_with_cols_final_export}
all(names(crbirding_birds_2020) == names(crbirding_birds_dead))
```

Merge `crbirding_birds_dead` and `crbirding_birds_klgev`:

```{r create_crbirding_birds}
crbirding_birds <- bind_rows(crbirding_birds_dead,
                             crbirding_birds_klgev) %>%
  arrange(bird_reference)
crbirding_birds
```
...
write_csv(
  crbirding_birds,
  file = here::here("data", "processed", "crbirding_birds_patch_20210511.csv"),
  na = ""
)
```
