---
title: "Mapping observation - patch"
...
Observations created or modified after Nov, 19 2020:

```{r after_export_obs}
obs <-
  obs %>%
  filter(WijzigDatum > as_date("2020-11-19"))
```
...
The field `user_role` is automatically set to `O`, observer:

```{r user_role}
users <-
  users %>%
  mutate(sovon_user_role = "O")
```
...
Save to csv file `crbirding_users_patch.csv` in `./data/processed/` adding suffix `patch` followed by the date, `20201119`:

```{r write_user_data_txt}
crbirding_users %>%
  write_csv(
    file = here::here("data", "processed", "crbirding_users_patch_20201119.csv"),
    na = ""
  )
```
...
birds <- 
  ...
  left_join(crbirding_birds_2020 %>%
              select(bird_reference, bird_shorthand, inscription),
            by = c("NummerDesc" = "inscription"))
birds %>%
  select(Nummer, NummerNieuw, NummerDesc, bird_shorthand, bird_reference)
```

Check that all observations have a valid `bird_reference`value:
...
Save to text file (comma separated value). We use suffix `patch` followed by date `20201119`:

```{r write_processed_obs_data}
write_csv(
  crbirding_observations,
  file = here::here("data", "processed", "crbirding_observations_patch_20201119.csv"),
  na = ""
)
```
