...
```
Order user data by `user_reference`:

```{r arrange_by_user_ref}
crbirding_users <-
  crbirding_users %>%
  arrange(user_reference)
```
...
First, remove the help columns `Datum` and `inscription` from `crbirding_birds_dead` as it is not needed anymore:
...
Check that both `crbirding_birds_dead` and `crbirding_birds_klgev` have some (number of) columns:
...
Merge `crbirding_birds_dead` and `crbirding_birds_klgev` and order data by `bird_reference`:
