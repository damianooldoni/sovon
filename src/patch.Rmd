...
obs <-
  obs %>%
  filter(WijzigDatum >= as_date("2020-12-08"))
```
...
Save to csv file `crbirding_users_patch.csv` in `./data/processed/` adding suffix `patch` followed by the date, `20201208`:

```{r write_user_data_txt}
crbirding_users %>%
  write_csv(
    file = here::here("data", "processed", "crbirding_users_patch_20201208.csv"),
    na = ""
  )
```
...
SOVON expert explicitly asked to fill `sovon_user_id` with `user_reference` values in user data:

```{r add_user_id}
obs_and_acts <-
  obs_and_acts %>%
  mutate(sovon_user_id = WaarnemerNummer)
```
...
obs_and_acts <-
  ...
  mutate(sovon_status_full_grown_bird = case_when(
    all(c("br", "vang") %in% acts) && (!is.na(br) | !is.na(vang)) ~ "N",
    LeeftijdCode == "PU" ~ "-",
    TRUE ~ "U"
  ))
```

Distinct values present in our observations:

```{r overview_sovon_status_full_grown_bird}
obs_and_acts %>%
  distinct(sovon_status_full_grown_bird)
```
...
obs_and_acts <-
  ...
  mutate(sovon_observation_condition = case_when(
    "dood" %in% acts && dood == "dood" ~ 5,
    "ziek" %in% acts && ziek == "ziek" | 
      sovon_user_reference %in% workers_bird_shelters$user_reference ~ 2,
    "rngkl" %in% acts && !is.na(rngkl) | 
      "rngme" %in% acts && !is.na(rngme) | 
      "vang" %in% acts && !is.na(vang) | 
      "vangl" %in% acts && !is.na(vangl) |
      "veld" %in% acts && !is.na(veld) |
      "me" %in% acts && !is.na(me) | 
      "meweg" %in% acts && !is.na(meweg) | 
      "klweg" %in% acts && !is.na(klweg) | 
      "br" %in% acts && !is.na(br) ~ 1
  ))
```
...
Values of `sovon_observation_condition` present in patch data:

```{r distinct_values_sovon_observation_condition}
unique(obs_and_acts$sovon_observation_condition)
```
...
As we have only field observations (`veld` or `dood` actions) `sovon_MRI` is by default set to 4:
...
Mapping in patch observation data:

```{r mapping_bird_age_ringing}
obs_and_acts %>%
  distinct(LeeftijdCode, sovon_bird_age_obs)
```
...
obs_and_acts <-
  ...
  mutate(sovon_observation_is_capture = if_else(
    "vang" %in% acts && !is.na(vang) | 
      "vangl" %in% acts && !is.na(vangl) | 
      "rngkl" %in% acts && !is.na(rngkl) | 
      "rngme" %in% acts && !is.na(rngme), "Y", "N"
  ))
```

In case of patch exports of field observations, the field `sovon_observation_is_capture` should always be `"N"`:

```{r overview_mapping_is_capture}
obs_and_acts %>%
  distinct(sovon_observation_is_capture) %>%
  pull(sovon_observation_is_capture) == "N"
```
...
write_csv(
  crbirding_observations,
  file = here::here("data", "processed", "crbirding_observations_patch_20201208.csv"),
  na = ""
)
```
...
Check that there are no filed observations of a bird after the its death:
