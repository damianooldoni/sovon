...
MRI is 4 for observations linked to action `me` (metal ring read)  `veld` (field observation, equiavlent of colror ring read):
...
obs_and_acts <- 
  obs_and_acts %>%
  mutate(sovon_MRI = ifelse(sovon_observation_reference %in% vang_no_change_color_ring, 
                            4, 
                            sovon_MRI)
)
```
...
MRI is 4 for observations linked to action `br` and not `klweg`:

```{r br_MRI_4}
obs_and_acts <- 
  obs_and_acts %>%
  mutate(sovon_MRI = ifelse(!is.na(br) & is.na(klweg), 4 , sovon_MRI))
```

Observations without MRI:

```{r check_obs_without_MRI}
obs_and_acts %>%
  filter(is.na(sovon_MRI)) %>%
  select(-sovon_melder) # for privacy reasons
```

## Mapping of INBO actions to MRI, condition, status

The fields  `sovon_MRI`, `sovon_condition`, `sovon_status` should cover all possible actions/observations. No observations should have all three fields empty:
...
obs_and_acts %>%
  select(acts, sovon_MRI, sovon_condition, sovon_status) %>%
  distinct()
```
...
obs_after_ringing <- 
  ...
  distinct() %>%
  mutate(sovon_bird_age_ringing = date_ringing) %>%
  select(-c(date_ringing, age_ringing))
```

Otherwise it cannot be assessed and the field will be left empty:
