...
obs_missing_latitude_longitude <- 
  ...
  select(Nummer,
         ...
         PlaatsToponym)
obs_missing_latitude_longitude %>%
  write_csv("missing_latitude_longitude_info.txt", na = "", 
            quote_escape = "double")
obs_missing_latitude_longitude
```
...
By field `PlaatsBreedtegraadRichtingCode` we can know whether the observation took place in the northern or southern emisphere. In case of southern emisphere, the latitude value should be negative.
...
By field `PlaatsLengtegraadRichtingCode` we can know whether the observation took place in the western or eastern emisphere. In case of western emisphere, the longitude value should be negative.
...
If present, the observations with valid `PlaatsLengtegraadRichtingCode` but without `PlaatsLengtegraadRichtingCode` should be further evaluated:
...
obs_and_acts <- 
  obs_and_acts %>% 
  mutate(
    sovon_observation_lng2 = case_when(
...
The observations of sick birds not anymore released should get condition equal to 5. These observations are linked to the very last `ziek` action and followed by an observation with action `dood`. Not only, the observer should be a worker of a bird shelter, in `workers_bird_shelters`.
...
last_sick_df <- 
  obs_and_acts %>%
  filter(sovon_user_reference %in% workers_bird_shelters$user_reference) %>%
  filter(ziek == "ziek") %>%
...
condition_5_obs_ref <- 
  ...
  filter(sovon_bird_reference %in% 
           last_sick_before_dead_df$sovon_bird_reference) %>%
  left_join(last_sick_before_dead_df,
...
Observations with `sovon_condition`= 5:
...
obs_and_acts <- 
  ...
  mutate(sovon_MRI = if_else(!is.na(set_MRI_5) & is.na(klweg) & is.na(veld), set_MRI_5, sovon_MRI))
```
