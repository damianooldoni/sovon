...
2. `rngkl`: MRI = 2 
3. `rngme` only: MRI = 1
5. other `vang`/`vangl`: MRI = 4 or 5
6. `vang`/`vangl` + `klweg`: MRI = 2 or 4
7. `vang`/`vangl` + `meweg`: MRI = 1 or 4

We initialize `sovon_MRI` by mapping first three situations:
...
mapping_acts_to_MRI <- 
  ...
  mutate(sovon_MRI = case_when(
    ...
    TRUE ~ 4 # apply default value
  )
)
mapping_acts_to_MRI
```

We apply this provisional mapping as provided by `mapping_acts_to_MRI`:
...
Observations linked to `vang`/`vangl` + `klweg`: 

```{r vang_vangl_+_klweg}
obs_and_acts %>%
  filter((!is.na(vang) | !is.na(vangl)) & klweg == "klweg") %>%
...
In case MRI = 5 it means that the date of these observations is the same of date of applying a new ring. However, as the previous color ring was absent, MRI should be 2 (= applying a new ring) instead of 5:

```{r set_2_instead_of_5_klweg}
obs_and_acts <- 
  obs_and_acts %>%
  mutate(sovon_MRI = case_when(
    sovon_MRI == 5 & (vang == "vang" | vangl == "vangl") & klweg == "klweg" ~ 2,
    TRUE ~ sovon_MRI
  )
)
```
...
```{r check_klweg_vang_of_vangl_set_to_2}
obs_and_acts %>%
  filter((!is.na(vang) | !is.na(vangl)) & klweg == "klweg") %>%
  select(sovon_bird_reference, 
         sovon_observation_reference, 
...
Obesrvations linked to combinations `vang`/`vangl` and  `meweg`:

```{r vang_of_vangl+meweg}
obs_and_acts %>%
  filter((!is.na(vang) | !is.na(vangl)) & meweg == "meweg") %>%
  select(sovon_bird_reference, 
         sovon_observation_reference, 
         vang, 
         vangl,
...
A new metal ring has been added while the color ring has been changed, justifying the MRI value 5:
...
As cr-birding and INBO databases are color oriented, we will leave MRI value 5.
...
Same holds true for observations with actions  `vangl`:
...
The field `sovon_bird_age_obs` should be filled with the age of the bird as mentioned by the observer. This field is present in `obs_and_acts` in field `Datum`:

```{r mapping_bird_age_ringing}
obs_and_acts <- 
  obs_and_acts %>%
  mutate(sovon_bird_age_obs = Datum)
```
...
obs_and_acts <- 
  ...
  mutate(sovon_observation_is_capture = if_else(
    !is.na(vang) | !is.na(vangl) | !is.na(rngkl) | !is.na(rngme), 1, 0
  )
)
```
...
We add the note `color_ring_added.` to observations linked to  `sovon_MRI`equal to 2 (action `rngkl` or  `vang`+`klweg`):
...
cr_obs_cols <- c(
  ...
  "bird_age_obs", "bird_sex", "bird_position", "bird_direction"
)
```
