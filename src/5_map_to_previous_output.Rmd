...
A patch, first, while waiting to solve issue [#129](https://github.com/inbo/cr-birding/issues/129):

```{r patch_solve_issue_129}
crbirding_birds_first_color_ring <-
  crbirding_birds_first_color_ring %>% 
  mutate(first_color_ring = if_else(is.na(first_color_ring) & 
                                      bird_reference == 5102,
                                    "RZAG",
                                    first_color_ring))
```
...
```{r birds_without_first_color_ring2}
crbirding_birds_first_color_ring %>%
  filter(is.na(first_color_ring))
```
...
Again, double checking the unicity of `new_bird_reference`:
...
Now we can safely copy the values of `new_bird_reference` to `bird_reference` and remove the help column and `bird_reference_2019:

```{r set_bird_ref_again}
crbirding_birds <- 
  crbirding_birds %>%
  select(-c(bird_reference, bird_reference_2019)) %>%
